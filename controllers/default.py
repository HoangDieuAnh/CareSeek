...
def settings():
    ...
    if(auth.user.User_Type == "Carer"):
		...
		url = URL('download', args=type.CV)
    elif(auth.user.User_Type == "Carer Seeker"):
		type = db(db.CareSeeker.UserId == auth.user.id).select().first()
    return dict(user=user, cvUrl=url, role=type)
...
def myProfile():
    ...
    url = ""
    return dict(user=user, subscribe=subscribe, same=same, my_clients=my_clients, co_worker=co_worker,overall=overall, re=re, pu=pu, per=per, comment=comment)
